...
AllHCForm::AllHCForm(QWidget *parent) : QWidget(parent) {
    ...
    connect(editDataButton, &QPushButton::clicked, this, &AllHCForm::onEditButtonClicked);
    connect(addDataButton, &QPushButton::clicked, this, &AllHCForm::onAddButtonClicked);
    connect(deleteDataButton, &QPushButton::clicked, this, &AllHCForm::onDeleteButtonClicked);
    connect(backButton, &QPushButton::clicked, this, &AllHCForm::onBackButtonClicked);
...
void AllHCForm::loadHCData() {
    ...
    if (model->rowCount() == 0) {
        showMessageBox("Error", "No HCollections found!", QMessageBox::Warning);
    }
}

void AllHCForm::showMessageBox(const QString &title, const QString &message, QMessageBox::Icon icon) {
